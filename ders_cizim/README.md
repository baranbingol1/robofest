...
1. Perception samples the lower camera image, classifies black/red/blue pixels, segments the visible line, and emits `visible`, `center_error`, `confidence`, `color_name`, and `matched_target`.
2. Control defaults to the red target, can be started with `red` or `blue`, and follows the visible common/target-colored line. The selected action is converted into clipped differential wheel speeds.
...
That world passes `--target=black` and `--goal-zones=black=0.80:0.36:0.18` to the controller. `black` is intentionally not sampled by training mode or sequence mode; it is only for direct smoke validation of the symbolic black texture.
